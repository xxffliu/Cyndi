...
double& vector3::operator[] ( unsigned int i)
{
    if (i > 2)
    {
        cout<<"ERROR in vector3::operator[]" <<endl<< "The method has been called with an illegal index i="<<i<<"."<< endl;
        exit(EXIT_FAILURE);
    }
    if (i == 0)
        return _vx;
...
void vector3::createOrthoVector(vector3 &res) const
{
    ...
    if ( ( IsNearZero(this->x())) && (IsNearZero(this->y())) )
    {
        ...
        {
            cout << "makeorthovec zero vector" << endl;
            exit(EXIT_FAILURE);
        }
        cO.SetX(1.0);
    }
...
const vector3 VUnit ( 1.0, 1.0, 1.0 ) ;
...
/* Calculate the minimal distance between 2 lines ab and cd*/
double MinDist_lines(vector3 a, vector3 c, vector3 b, vector3 d)
{
       return fabs(dot(b-a, cross(c,d)))/cross(c,d).length();
}
//} // namespace OpenBabel
